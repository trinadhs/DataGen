...
public class ExportAttributes2 implements DropdownData{
		...
		try { 
			...
			for (int i = 1; i <= noOfRow; i++) {
				...
				result= data.add(new String[] { Ref_attrib_tmpl+i,attrib_tmpl+i,"This is a defination of "+Segment_name+" Attribute"+i,
						...
						Segment_name+((i%noOfpeoples)+1)+"@informatica.com",Segment_name+" First Name "+((i%noOfpeoples)+1),Segment_name+" Last Name "+((i%noOfpeoples)+1),"0.0.0."+i,Attribute_GovernanceRole[randGovernanceRole] });
			...
			if(result) {
				System.out.println("Attribute file Upload pass");
			}
